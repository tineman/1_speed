...
function roleAdapter(index, role) {
    if (role === CONSTANTS.OTHER) {
        ...
        else if (index === CONSTANTS.OTHER_DECK)
            return CONSTANTS.SELF_DECK; //maps OTHER deck to SELF deck
        else if (index === CONSTANTS.SELF_DECK)
            return CONSTANTS.OTHER_DECK; //vice versa
    }
    return index;
}
/**
 ...
 * @param card
 */
export function updateHTML(cards, role) {
    for (let i = 0; i < 14; i++) {
        //Normal card
        let card = cards[i];
        if (card.rank > 0 && card.rank < 14) {
            //@ts-ignore
            if (card.faceup)
                gameElements[roleAdapter(i, role)].context.drawImage(img, ((card.rank - 1) * 18), ((card.suit - 1) * 22), 18, 22, 0, 0, CONSTANTS.CARD_WIDTH, CONSTANTS.CARD_HEIGHT);
            //@ts-ignore
            else
                gameElements[roleAdapter(i, role)].context.drawImage(img, 179, 96, 18, 22, 0, 0, CONSTANTS.CARD_WIDTH, CONSTANTS.CARD_HEIGHT);
        }
        switch (card.rank) {
 ...
 * @param role
 */
export function updateSelect(indices, role) {
