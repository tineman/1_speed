 ...
 * @param card
 */
export function updateHTML(index, card) {
    //Normal card
    if (card.rank > 0 && card.rank < 14) {
        //@ts-ignore
        if (card.faceup)
            gameElements[index].context.drawImage(img, ((card.rank - 1) * 18), ((card.suit - 1) * 22), 18, 22, 0, 0, CONSTANTS.CARD_WIDTH, CONSTANTS.CARD_HEIGHT);
        //@ts-ignore
        else
            gameElements[index].context.drawImage(img, 179, 96, 18, 22, 0, 0, CONSTANTS.CARD_WIDTH, CONSTANTS.CARD_HEIGHT);
    }
    switch (card.rank) {
